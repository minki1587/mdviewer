...
| 모두 저장 | `Ctrl+Alt+S` |
| PDF로 내보내기 | `Ctrl+P` — 위쪽 바의 내보내기 단추도 같습니다 |
...
`Ctrl+P` 또는 위쪽 바의 내보내기 단추를 누르면 인쇄 대화상자가 열립니다.
프린터 대신 **PDF로 저장(Microsoft Print to PDF)** 을 고르면 그대로 PDF 파일이
됩니다. 나가는 것은 지금 탭의 **읽기 화면**입니다 — 편집 중이었다면 최신 원문을
다시 그려서 내보내므로, 미리 읽기 모드로 바꿔 둘 필요가 없습니다.

종이에는 상단 바·탭·목차·편집 창이 빠지고 본문만 나갑니다. 어두운 화면으로
보고 있어도 흰 바탕에 검은 글씨로 찍히고, 화면에서 옆으로 스크롤하던 긴 코드
줄과 넓은 표는 줄을 접어 페이지 안에 담습니다. 접어 둔 `<details>` 는 인쇄
직전에 펼쳐지고, 아직 화면에 안 나온 그림도 기다렸다가 함께 내보냅니다.
...
| 찾기·바꾸기 | `Ctrl+F` — 편집 중에는 편집기의 찾기·바꾸기가 열립니다 |
